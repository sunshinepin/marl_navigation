...
class MATD3(object):
    def __init__(self, env_info, max_action):
        ...
        self.critics = {agent: Critic(state_dim, action_dim).to(device) for agent in self.agents}
...
if __name__ == "__main__":
    ...
    save_model = True
    load_model = False
    random_near_obstacle = True  # 启用靠近障碍物的随机动作
    ...
    episode_num = 0
    epoch = 1
    count_rand_actions = {agent: 0 for agent in env_info['agents']}  # 每个智能体的随机动作计数器
    random_actions = {agent: None for agent in env_info['agents']}  # 每个智能体的随机动作
