...
from helper_functions import check_pos, generate_unique_goal, observe_collision, get_reward
from visualization_helpers import publish_markers
...
class GazeboEnv:
    ...
    def step(self, actions):
        ...
        for i, (agent, action) in enumerate(actions.items()):
            if not self.target_reached[i]:
                try:
                    ...
                    publish_markers(i, action, self.goal_positions, self.publishers, self.publishers2, self.publishers3, self.car_names)
        ...
        for i in range(len(self.car_names)):
            done, collision, min_laser = observe_collision(self.velodyne_data[i])
            v_state = []
            ...
            reward = get_reward(self.target_reached[i], collision, action, min_laser, distance)
    ...
    def reset(self):
        ...
        for i in range(len(self.car_names)):
            ...
            publish_markers(i, [0.0, 0.0], self.goal_positions, self.publishers, self.publishers2, self.publishers3, self.car_names)
