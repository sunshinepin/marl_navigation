...
from velodyne_env_sum2 import GazeboEnv
...
environment_dim = 20
robot_dim = 4
env1 = GazeboEnv("td3.launch", environment_dim, "car1", -8.0, 5.0, 0.01)
time.sleep(5)
...
if load_model:
    try:
        ...
        print("Could not load the stored model parameters for car1, initializing training with random parameters")

# Create the training environment and network for car2
env2 = GazeboEnv("td3.launch", environment_dim, "car2", -8.0, -5.0, 0.01)
time.sleep(5)
...
if load_model:
    try:
        ...
        print("Could not load the stored model parameters for car2, initializing training with random parameters")
...
while timestep < max_timesteps:
    ...
    if done1:
        ...
        if timesteps_since_eval >= eval_freq:
            print("Validating car1")
            timesteps_since_eval %= eval_freq
            evaluations1.append(
                evaluate(network=network1, env=env1, epoch=epoch, eval_episodes=eval_ep)
            )
    ...
    done1 = 1 if episode_timesteps1 + 1 == max_ep else int(done1)
    ...
    if done2:
        ...
        if timesteps_since_eval >= eval_freq:
            print("Validating car2")
            timesteps_since_eval %= eval_freq
            evaluations2.append(
                evaluate(network=network2, env=env2, epoch=epoch, eval_episodes=eval_ep)
            )
    ...
    done2 = 1 if episode_timesteps2 + 1 == max_ep else int(done2)
...
evaluations1.append(evaluate(network=network1, env=env1, epoch=epoch, eval_episodes=eval_ep))
evaluations2.append(evaluate(network=network2, env=env2, epoch=epoch, eval_episodes=eval_ep))
