...
from DistributedGazeboEnv import DistributedGazeboEnv
...
class ParameterServer:
    def __init__(self, state_dim, action_dim):
        ...
        self.lock = threading.Lock()  # 为更新参数添加锁
...
class Worker:
    def __init__(self, replay_buffer, param_server, state_dim, action_dim, max_action, environment_dim, car_name, car_position, car_orientation, gamma=0.99, policy_noise=0.2, noise_clip=0.5, policy_freq=2):
        self.env = DistributedGazeboEnv("td3.launch", environment_dim, [car_name], [car_position], [car_orientation])
        ...
        for episode in range(max_episodes):
            try:
                ...
                while not done:
                    ...
                    ray.get(self.replay_buffer.add.remote(state, action, reward, done, next_state))
                    state = next_state
