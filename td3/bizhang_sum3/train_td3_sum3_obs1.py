...
class TD3(object):
    ...
    def train(
        ...
        self.iter_count += 1
        self.writer.add_scalar(f"{self.name}/av_loss", av_loss / iterations, self.iter_count)
        self.writer.add_scalar(f"{self.name}/loss", loss.item(), self.iter_count)
        self.writer.add_scalar(f"{self.name}/Av. Q", av_Q / iterations, self.iter_count)
        self.writer.add_scalar(f"{self.name}/Max. Q", max_Q, self.iter_count)
...
# Initial positions and orientations for the cars
car_positions = [[0.0, 6.0, 0.01], [0.0, -6.0, 0.01], [-6.0, 0.0, 0.01]]
car_orientations = [-1.57, 1.57, 0]  # These match the yaw values in your launch file
...
# Pass positions and orientations to the Gazebo environment
...
log_file_name = f"training_log_{current_date}.txt"
...
while timestep < max_timesteps:
    ...
    if (done1 and done2 and done3) or (episode_timesteps1 >= max_ep and episode_timesteps2 >= max_ep and episode_timesteps3 >= max_ep):
        ...
        if timesteps_since_eval >= eval_freq:
            ...
            evaluations1.append(
                evaluate(networks=[network1, network2, network3], env=env, epoch=epoch, eval_episodes=eval_ep)
            )
        ...
        # Reset environment and pass the initial positions and orientations
        states = env.reset()  # This reset will use the car_positions and car_orientations provided during initialization
    ...
    actions = {f"agent_{i}": (network.get_action(np.array(state)) + np.random.normal(0, expl_noise, size=action_dim)).clip(-max_action, max_action) for i, (network, state) in enumerate(zip([network1, network2, network3], states.values()))}
...
evaluations1.append(evaluate(networks=[network1, network2, network3], env=env, epoch=epoch, eval_episodes=eval_ep))
