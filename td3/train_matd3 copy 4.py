...
policy_freq = 2
buffer_size = 1e6
file_name = "TD3_multi_agent"  # 用于保存新模型
save_model = True
load_model = True
random_near_obstacle = True

# 预训练模型路径
pretrained_actor_path = "./pytorch_models/TD3_velodyne_actor.pth"
pretrained_critic_path = "./pytorch_models/TD3_velodyne_critic.pth"
...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, old_state_dim=24):
        ...
        self.adapt_layer = nn.Linear(state_dim, old_state_dim)
        self.layer_1 = nn.Linear(old_state_dim, 800)
        self.layer_2 = nn.Linear(800, 600)
        self.layer_3 = nn.Linear(600, action_dim)
        self.tanh = nn.Tanh()

    def forward(self, s):
        s = F.relu(self.adapt_layer(s))
...
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim, old_state_dim=24):
        ...
        self.layer_4 = nn.Linear(old_state_dim, 800)
...
class TD3(object):
    ...
    def load(self):  # 修改为直接使用预训练路径
        try:
            old_actor_dict = torch.load(pretrained_actor_path)
            old_critic_dict = torch.load(pretrained_critic_path)
            ...
            for name, param in self.critic.named_parameters():
                if "adapt_layer" not in name:
                    param.requires_grad = False
            print("Loaded pre-trained model successfully from", pretrained_actor_path, "and", pretrained_critic_path)
        except Exception as e:
            print(f"Could not load pre-trained model: {e}. Initializing with random parameters.")
        ...
        for agent in env.agents:
            if not done[agent]:
                ...
                if random_near_obstacle:
                    if (np.random.uniform(0, 1) > 0.85 and min(state[4:-8]) < 0.6 and
